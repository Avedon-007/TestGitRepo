...
public class Task_11 {
	...
	public static void main(String[] args) 
	{
		...
		superArray.showArray(superArray.createArray(4));
	}
...
public int[][] createArray(int dimension)
{
	...
	for(int i = 3; i <= dimension*dimension; i++)
	{
		...
		if((directionFlag = true) && (xPosition< 0)) // upper border
		{
			xPosition++;
			directionFlag = false;
			
		}
		else if((directionFlag = true) && (yPosition> dimension-1))	// right wall(border)
		{
			xPosition+=2;
			yPosition--;
			directionFlag = false;
